...
func setupHookFunctions() async throws {
    ...
    // When ready to upgrade, create version 2
    var functionV2 = ParseHookFunction(
        name: "processOrder",
        url: URL(string: "https://api.example.com/v2/functions/processOrder")!
    )
    
    // Update the Hook Function to point to v2
    try await functionV2.update()
